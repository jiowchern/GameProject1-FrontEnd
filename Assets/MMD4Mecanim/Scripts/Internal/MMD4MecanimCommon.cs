...
//#if UNITY_WEBPLAYER
				...
				if( (int)fileStream.Length >= 18 ) {
					...
					if( ( cMapType == 0 || cMapType == 1 ) && ( origin == 0 || origin == 2 ) ) {
						...
						if( ( imageType == 2 || imageType == 10 ) && ( pixel == 16 || pixel == 24 || pixel == 32 ) ) {
							if( pixel == 24 ) {
								return TextureFileSign.Targa; // FIXED: Counts as Targa.(TGA support only not alpha.)
							}
							if( pixel == 32 ) {
								return TextureFileSign.TargaWithAlpha; // FIXED: Counts as TargaWithAlpha.(alphaBits == 0 in anyware)
							}
	...
	public static CloneMeshWork CloneMesh( Mesh sharedMesh )
	{
		...
		cloneMeshWork.uv1 = sharedMesh.uv1;
		...
		cloneMeshWork.triangles = sharedMesh.triangles; // Memo: Maybe don't use.
		...
		mesh.triangles = cloneMeshWork.triangles; // Memo: Maybe don't use.
