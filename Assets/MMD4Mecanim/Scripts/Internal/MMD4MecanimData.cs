...
public static class MMD4MecanimData
{
	...
	public const int FullBodyIKTargetMax =
			(int)FullBodyIKBodyTarget.Max
			+ (int)FullBodyIKArmTarget.Max * 2
			+ (int)FullBodyIKLegTarget.Max * 2
			+ (int)FullBodyIKFingerTarget.Max * 2
			+ (int)FullBodyIKLookTarget.Max;

	static bool _Decl( ref int index, int max )
	{
		if( index < max ) {
			return true;
		}
	...
	{
		if( fullBodyIKTargetIndex >= 0 && fullBodyIKTargetIndex < FullBodyIKTargetMax ) {
			if( _Decl( ref fullBodyIKTargetIndex, (int)FullBodyIKBodyTarget.Max ) ) {
				return FullBodyIKGroup.Body;
			}
			if( _Decl( ref fullBodyIKTargetIndex, (int)FullBodyIKArmTarget.Max ) ) {
				return FullBodyIKGroup.LeftArm;
			}
			if( _Decl( ref fullBodyIKTargetIndex, (int)FullBodyIKArmTarget.Max ) ) {
				return FullBodyIKGroup.RightArm;
			}
			if( _Decl( ref fullBodyIKTargetIndex, (int)FullBodyIKLegTarget.Max ) ) {
				return FullBodyIKGroup.LeftLeg;
			}
			if( _Decl( ref fullBodyIKTargetIndex, (int)FullBodyIKLegTarget.Max ) ) {
				return FullBodyIKGroup.RightLeg;
			}
			if( _Decl( ref fullBodyIKTargetIndex, (int)FullBodyIKFingerTarget.Max ) ) {
				return FullBodyIKGroup.LeftWristFinger;
			}
			if( _Decl( ref fullBodyIKTargetIndex, (int)FullBodyIKFingerTarget.Max ) ) {
				return FullBodyIKGroup.RightWristFinger;
			}
			if( _Decl( ref fullBodyIKTargetIndex, (int)FullBodyIKLookTarget.Max ) ) {
	...
	public static string GetFullBodyIKTargetName( out FullBodyIKGroup fullBodyIKGroup, int fullBodyIKTargetIndex )
	{
		...
		if( fullBodyIKTargetIndex >= 0 && fullBodyIKTargetIndex < FullBodyIKTargetMax ) {
			...
			case FullBodyIKGroup.Body:
				return ((FullBodyIKBodyTarget)fullBodyIKTargetIndex).ToString();
			case FullBodyIKGroup.LeftArm:
				return ((FullBodyIKArmTarget)fullBodyIKTargetIndex).ToString();
			case FullBodyIKGroup.RightArm:
				return ((FullBodyIKArmTarget)fullBodyIKTargetIndex).ToString();
			case FullBodyIKGroup.LeftLeg:
				return ((FullBodyIKLegTarget)fullBodyIKTargetIndex).ToString();
			case FullBodyIKGroup.RightLeg:
				return ((FullBodyIKLegTarget)fullBodyIKTargetIndex).ToString();
			case FullBodyIKGroup.LeftWristFinger:
				return ((FullBodyIKFingerTarget)fullBodyIKTargetIndex).ToString();
			case FullBodyIKGroup.RightWristFinger:
				return ((FullBodyIKFingerTarget)fullBodyIKTargetIndex).ToString();
			case FullBodyIKGroup.Look:
				return ((FullBodyIKLookTarget)fullBodyIKTargetIndex).ToString();
			}
		}
	...
	public class ModelData
	{
		...
		public Dictionary< string, int > morphDataDictionary { get { return morphDataDictionaryJp; } }

		public bool isMorphTranslateName {
			get {
				return (this.modelAdditionalFlags & ModelAdditionalFlags.MorphTranslateName) != ModelAdditionalFlags.None;
			}
		}
		...
		public int GetMorphDataIndexJp( string morphName, bool isStartsWith )
		{
			if( morphName != null && this.morphDataList != null ) {
				if( morphDataDictionaryJp != null && morphDataDictionaryJp.Count > 0 ) {
					int morphIndex = 0;
					if( morphDataDictionaryJp.TryGetValue( morphName, out morphIndex ) ) {
						return morphIndex;
					}
					if( isStartsWith ) {
		...
		{
			if( morphName != null && this.morphDataList != null ) {
				if( morphDataDictionaryEn != null && morphDataDictionaryEn.Count > 0 ) {
					int morphIndex = 0;
					if( morphDataDictionaryEn.TryGetValue( morphName, out morphIndex ) ) {
						return morphIndex;
					}
					if( isStartsWith ) {
						for( int i = 0; i < this.morphDataList.Length; ++i ) {
							if( this.morphDataList[i].nameEn != null &&
							   this.morphDataList[i].nameEn.StartsWith( morphName ) ) {
								return i;
							}
						}
					}
		...
		{
			if( morphName != null && this.morphDataList != null ) {
				if( translatedMorphDataDictionary != null && translatedMorphDataDictionary.Count > 0 ) {
					int morphIndex = 0;
					if( translatedMorphDataDictionary.TryGetValue( morphName, out morphIndex ) ) {
						return morphIndex;
					}
					if( isStartsWith ) {
						for( int i = 0; i < this.morphDataList.Length; ++i ) {
							if( this.morphDataList[i].translatedName != null &&
							   this.morphDataList[i].translatedName.StartsWith( morphName ) ) {
								return i;
							}
						}
					}
	...
	*/
	...
	private static bool _ParseMorphData( ModelData modelData, MMD4MecanimCommon.BinaryReader binaryReader )
	{
		...
		for( int structIndex = 0; structIndex < binaryReader.currentStructLength; ++structIndex ) {
			...
			int nameEn = binaryReader.ReadStructInt();
			...
			int translateName = binaryReader.ReadStructInt();
			
			morphData.nameJp		= binaryReader.GetName( nameJp );
			morphData.nameEn		= binaryReader.GetName( nameEn );
			if( modelData.isMorphTranslateName ) {
				morphData.translatedName = binaryReader.GetName( translateName );
			}
			...
			if( !string.IsNullOrEmpty( morphData.nameJp ) ) {
				modelData.morphDataDictionaryJp[morphData.nameJp] = structIndex;
			}
			if( !string.IsNullOrEmpty( morphData.nameEn ) ) {
				modelData.morphDataDictionaryEn[morphData.nameEn] = structIndex;
			}
			if( modelData.isMorphTranslateName ) {
				if( !string.IsNullOrEmpty( morphData.translatedName ) ) {
					modelData.translatedMorphDataDictionary[morphData.translatedName] = structIndex;
				}
			}
		}
