		...
		if( _nextEdgeVisibleCached != isVisible ) {
			...
			if( _nextEdgeMeshRenderers != null ) {
				for( int i = 0; i != _nextEdgeMeshRenderers.Length; ++i ) {
					_nextEdgeMeshRenderers[i].enabled = isVisible;
				}
			}
			if( _nextEdgeSkinnedMeshRenderers != null ) {
				for( int i = 0; i != _nextEdgeSkinnedMeshRenderers.Length; ++i ) {
					_nextEdgeSkinnedMeshRenderers[i].enabled = isVisible;
				}
			}
		}
