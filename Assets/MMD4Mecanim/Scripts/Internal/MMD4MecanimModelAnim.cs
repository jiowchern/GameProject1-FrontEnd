...
public partial class MMD4MecanimModel
{
	...
	void _InitializeAnimatoion()
	{
		...
		// Memo: Support delayedAwakeFrame.
		if( Application.isPlaying ) {
			if( this.animEnabled && this.animDelayedAwakeFrame > 0 ) {
				if( _animator != null && _animator.enabled ) {
					_animator.speed = 0.0f; // Memo: Delayed awake.
	...
	// Memo: Support delayedAwakeFrame.
	int _delayedAwakeFrame;
	bool _isDelayedAwake;
	...
	void _UpdateAnim()
	{
		...
		// Memo: Support delayed awake.
		if( _bulletPhysicsMMDModel == null ) {
			return;
		}
		if( _isDelayedAwake ) {
			if( this.animEnabled && this.animDelayedAwakeFrame > 0 ) {
				if( ++_delayedAwakeFrame <= this.animDelayedAwakeFrame ) {
					return;
				}
			}
