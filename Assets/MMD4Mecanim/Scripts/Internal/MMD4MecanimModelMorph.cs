...
public partial class MMD4MecanimModel
{
	...
	{
		// isStartsWith for VMD.
		if( this.modelData != null ) {
			int morphIndex = this.modelData.GetMorphDataIndexJp( morphName, isStartsWith );
			if( morphIndex != -1 ) {
				return this.morphList[morphIndex];
			}

			if( !isStartsWith ) {
				morphIndex = this.modelData.GetMorphDataIndexEn( morphName, false );
				if( morphIndex != -1 ) {
					return this.morphList[morphIndex];
				}

				morphIndex = this.modelData.GetTranslatedMorphDataIndex( morphName, false );
				if( morphIndex != -1 ) {
					return this.morphList[morphIndex];
				}
			}
	...
	void _ApplyMaterialData( ref MMD4MecanimData.MorphMaterialData morphMaterialData, float weight )
	{
		if( _cloneMaterials != null ) {
			for( int n = 0; n != _cloneMaterials.Length; ++n ) {
				CloneMaterial cloneMaterial = _cloneMaterials[n];
