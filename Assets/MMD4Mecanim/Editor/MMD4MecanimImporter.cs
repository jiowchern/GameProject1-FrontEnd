...
public partial class MMD4MecanimImporter : ScriptableObject
{
	...
	public void OnInspectorGUI()
	{
		...
		if( this.pmx2fbxConfig != null ) {
			if( _licenseAgree == LicenseAgree.NotProcess ) {
				_licenseAgree = CheckLicenseAgree( ref _readmeFiles, ref _readmeTexts, ref _isPiapro, ref _isNitro, ref _isKancolle );
			...
			if( !_isAgreedWeak ) {
...

				{
					...
					string str_LicenceWarningMorality = LicenseWarningMorality;
					string str_LicenceDeniedMorality = LicenseDeniedMorality;
					string str_LicenceMorality = LicenseMorality;
					string str_LicenseWarningPiapro = LicenseWarningPiapro;
					string str_LicenseWarningPiaproLink = LicenseWarningPiaproLink;
					string str_LicenseWarningNitro = LicenseWarningNitro;
					string str_LicenseWarningNitroLink = LicenseWarningNitroLink;
					string str_LicenseWarningKancolle = LicenseWarningKancolle;
					...
					if( Application.systemLanguage != SystemLanguage.Japanese ) {
						...
						str_LicenceWarningMorality = LicenseWarningMorality_En;
						str_LicenceDeniedMorality = LicenseDeniedMorality_En;
						str_LicenceMorality = LicenseMorality_En;
						str_LicenseWarningPiapro = LicenseWarningPiapro_En;
						str_LicenseWarningPiaproLink = LicenseWarningPiaproLink_En;
						str_LicenseWarningNitro = LicenseWarningNitro_En;
						str_LicenseWarningNitroLink = LicenseWarningNitroLink_En;
						str_LicenseWarningKancolle = LicenseWarningKancolle_En;
					...
					if( _licenseAgree != LicenseAgree.Denied ) {
						EditorGUILayout.LabelField( str_LicenceWarningNotice );
						EditorGUILayout.Separator();
					}
					...
					if( _licenseAgree != LicenseAgree.Denied ) {
						if( _isPiapro || _isNitro ) {
							_isCheckedMorality = GUILayout.Toggle(_isCheckedMorality, str_LicenceDeniedMorality);
						} else {
							_isCheckedMorality = GUILayout.Toggle(_isCheckedMorality, str_LicenceWarningMorality);
						}
						{
							EditorGUILayout.BeginHorizontal();
							GUILayout.Space( 18.0f );
							EditorGUILayout.LabelField( str_LicenceMorality );
							EditorGUILayout.EndHorizontal();
							EditorGUILayout.Separator();
						}
					}
					if( _isPiapro ) {
						...
						EditorGUILayout.BeginHorizontal();
						GUILayout.Space( 18.0f );
						if( GUILayout.Button( str_LicenseWarningPiaproLink ) ) {
							System.Diagnostics.Process.Start( str_LicenseWarningPiaproLink );
						}
						EditorGUILayout.EndHorizontal();
						EditorGUILayout.Separator();
					...
					if( _isNitro ) {
						_isCheckedNitro = GUILayout.Toggle(_isCheckedNitro, str_LicenseWarningNitro);
						EditorGUILayout.BeginHorizontal();
						GUILayout.Space( 18.0f );
						if( GUILayout.Button( str_LicenseWarningNitroLink ) ) {
							System.Diagnostics.Process.Start( str_LicenseWarningNitroLink );
						}
						EditorGUILayout.EndHorizontal();
						EditorGUILayout.Separator();
						licenseAgreeded = licenseAgreeded && _isCheckedNitro;
					}
					if( _isKancolle ) {
						_isCheckedKancolle = GUILayout.Toggle(_isCheckedKancolle, str_LicenseWarningKancolle);
						EditorGUILayout.Separator();
						licenseAgreeded = licenseAgreeded && _isCheckedKancolle;
					}
					if( _licenseAgree == LicenseAgree.Denied ) {
	...
	public static readonly string[] LicenseNitroStrings = new string[] {
		"\u30CB\u30C8\u30ED\u30D7\u30E9\u30B9",
		"nitroplus",
		"Nitroplus",
	};

	public static readonly string[] LicenseKancolleStrings = new string[] {
		"\u8266\u968a\u3053\u308c\u304f\u3057\u3087\u3093",
		"\u8266\u3053\u308c",
	};
	...
	public static readonly string LicenseWarningKancolle_En = "Public order and morality in contrary representation,\nand has been restricted for distribution of content that game properties.";
	public static readonly string LicenseWarningMorality_En = "Please consider public order and morals enough.";
	public static readonly string LicenseDeniedMorality_En = "The expression against public order and morals is prohibited.";
	public static readonly string LicenseMorality_En = "(When a family watches it all together, please warn him not to let you hold perplexity, an aversive feeling by doing open expression description.)";
	...
	public LicenseAgree CheckLicenseAgree( ref string[] readmeFiles, ref string[] readmeTexts, ref bool isPiapro, ref bool isNitro, ref bool isKancolle )
	{
		readmeFiles = FindReadmeFiles();
		readmeTexts = null;
		return CheckLicenseAgree( readmeFiles, ref readmeTexts, ref isPiapro, ref isNitro, ref isKancolle );
	}
	...
	public static LicenseAgree CheckLicenseAgree( string[] readmeFiles, ref string[] readmeTexts, ref bool isPiapro, ref bool isNitro, ref bool isKancolle )
	{
		isPiapro = false;
		isKancolle = false;
		...
		for( int n = 0; n < readmeFiles.Length; ++n ) {
			...
			try {
				...
				for( int i = 0; i < LicenseNitroStrings.Length; ++i ) {
					if( readmeTexts[n].Contains( LicenseNitroStrings[i] ) ) {
						isNitro = true;
						break;
					}
				}
				for( int i = 0; i < LicenseKancolleStrings.Length; ++i ) {
					if( readmeTexts[n].Contains( LicenseKancolleStrings[i] ) ) {
						isKancolle = true;
						break;
					}
				}
