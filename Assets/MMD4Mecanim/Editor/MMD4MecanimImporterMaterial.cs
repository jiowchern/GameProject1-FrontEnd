...
{
	public const float ShaderRevision = 2.0f;
	// 1.0 ... Support Shader Keywords.
	// 2.0 ... Bugfix SPECULAR_ON
	...
	public class MaterialBuilder
	{
		...
		private static string _PrefixFileName( string fileName )
		{
			if( fileName != null ) {
				...
				return _PrefixFileName2( fileName.Substring( 0, endPos + 1 ) );
		...
		public void Build( bool overwriteMaterials )
		{
			...
			bool prefixRenderQueue = true;
			bool renderQueueAfterSkybox = false;
			Color baseDiffuse = new Color( 1.0f, 1.0f, 1.0f, 1.0f );
			float ambientRate = 1.0f;
			float ambientToDiffuse = 0.2f;
			float edgeScale = 1.0f;
			float shadowLum = 1.25f;
			bool isSpecular = true;
			bool isSelfShadow = false;
			float addLightToonCen = 0.0f;
			float addLightToonMin = 0.0f;
			bool materialMorph = true;
			PMX2FBXConfig.AutoLuminous autoLuminous = PMX2FBXConfig.AutoLuminous.Disable;
			float autoLuminousPower = 0.0f;
			if( this.mmd4MecanimProperty != null ) {
				prefixRenderQueue = this.mmd4MecanimProperty.prefixRenderQueue;
				renderQueueAfterSkybox = this.mmd4MecanimProperty.renderQueueAfterSkybox;
				baseDiffuse = this.mmd4MecanimProperty.baseDiffuse;
				ambientRate = this.mmd4MecanimProperty.ambientRate;
				ambientToDiffuse = this.mmd4MecanimProperty.ambientToDiffuse;
				edgeScale = this.mmd4MecanimProperty.edgeScale;
				shadowLum = this.mmd4MecanimProperty.shadowLum;
				isSpecular = this.mmd4MecanimProperty.isSpecular;
				isSelfShadow = this.mmd4MecanimProperty.isSelfShadow;
				...
				materialMorph = this.mmd4MecanimProperty.materialMorph;
			}
			...
			this.materialList = MMD4MecanimImporter._GetFBXMaterialList( this.fbxAsset, this.mmdModel );
			...
			if( this.materialList != null ) {
				...
				for( int i = 0; i < this.materialList.Count; ++i ) {
					...
					if( materialMorph ) {
						// Added: Under Material Morph
						if( this.mmdModel.morphList != null ) {
							for( int n = 0; n < this.mmdModel.morphList.Length; ++n ) {
								if( this.mmdModel.morphList[n].morphType == MMDModel.MorphType.Material ) {
									if( this.mmdModel.morphList[n].indexList != null ) {
										for( int m = 0; m < this.mmdModel.morphList[n].indexList.Length; ++m ) {
											int idx = this.mmdModel.morphList[n].indexList[m];
											if( idx == i ) {
												materialIsTransparency[i] = true;
											}
										}
				...
				for( int i = 0; i < this.materialList.Count; ++i ) {
					...
					bool isSupportUnity4 = false;
					#if UNITY_4_0 || UNITY_4_1 || UNITY_4_2 || UNITY_4_3 || UNITY_4_4 || UNITY_4_5 || UNITY_4_6
					if( _Contains( material.shaderKeywords, "UNITY4SUPPORT_ON" ) ) {
						isSupportUnity4 = true;
					}
					if( !isSupportUnity4 ) {
						material.EnableKeyword("UNITY4SUPPORT_ON");
						material.DisableKeyword("UNITY4SUPPORT_OFF");
					}
					#else
					if( material != null && material.IsKeywordEnabled("UNITY4SUPPORT_ON") ) {
						isSupportUnity4 = true;
					}
					if( isSupportUnity4 ) {
						material.EnableKeyword("UNITY4SUPPORT_OFF");
						material.DisableKeyword("UNITY4SUPPORT_ON");
					}
					#endif
					...
					if( shaderType == (int)PMX2FBXConfig.ShaderType.MMD4Mecanim ) {
						if( initializedShader ) {
							...
							material.SetFloat("_AmbientRate", ambientRate);
							float a2dMin = 0.05f;
							material.SetFloat("_AmbientToDiffuse", Mathf.Min(
								(ambientToDiffuse > a2dMin) ? (1.0f / ambientToDiffuse) : (1.0f / a2dMin), 1.0f / a2dMin ) );
							if( ambientToDiffuse <= a2dMin ) {
								material.EnableKeyword( "AMB2DIFF_OFF" );
								material.DisableKeyword( "AMB2DIFF_ON" );
							} else {
								material.DisableKeyword( "AMB2DIFF_OFF" );
								material.EnableKeyword( "AMB2DIFF_ON" );
							}
						...
						if( initializedShader || shaderRevision < ShaderRevision - 0.5f ) {
							if( isSpecular && xmlMaterial.shiness > 0.0f ) {
								material.DisableKeyword( "SPECULAR_OFF" );
								material.EnableKeyword( "SPECULAR_ON" );
							} else {
								material.EnableKeyword( "SPECULAR_OFF" );
						...
						if( initializedShader ) {
							...
							if( autoLuminous == PMX2FBXConfig.AutoLuminous.Emissive ) {
								...
								if( shiness > 100.0f ) {
									...
									material.SetFloat( "_ALPower", autoLuminousPower );
									material.EnableKeyword( "EMISSIVE_OFF" );
									material.DisableKeyword( "EMISSIVE_ON" );
								}
							} else {
								material.SetColor( "_Emissive", new Color(0,0,0,0) );
						...
						if( initializedShader || shaderRevision < ShaderRevision - 0.5f ) {
							...
							if( xmlMaterial.sphereMode != MMDModel.SphereMode.None ) {
								if( additionalTexture != null ) {
									...
									if( SetTextureCubemap( additionalTexture, TextureImporterGenerateCubemap.Spheremap ) ) {
										...
										material.DisableKeyword( "SPHEREMAP_OFF" );
										material.DisableKeyword( "SPHEREMAP_MUL" );
										material.DisableKeyword( "SPHEREMAP_ADD" );
									} else {
									...
									material.EnableKeyword( "SPHEREMAP_OFF" );
									material.DisableKeyword( "SPHEREMAP_MUL" );
									material.DisableKeyword( "SPHEREMAP_ADD" );
								}
							} else {
							...
							if( xmlMaterial.sphereMode != MMDModel.SphereMode.None && additionalTexture != null ) {
								if( xmlMaterial.sphereMode == MMDModel.SphereMode.Multiply ) {
									...
									material.EnableKeyword( "SPHEREMAP_MUL" );
									...
									material.EnableKeyword( "SPHEREMAP_ADD" );
					...
					if( shaderType == (int)PMX2FBXConfig.ShaderType.MMD4Mecanim ) {
						if( prefixRenderQueue ) {
							int baseRenderQueue = renderQueueAfterSkybox
								? MMD4MecanimModel.MaterialBaseRenderQueue_AfterSkybox
								: MMD4MecanimModel.MaterialBaseRenderQueue;
	...
	private static int _ToMaterialIndex( MMDModel mmdModel, string materialName )
	{
		if( mmdModel != null && mmdModel.materialList != null && !string.IsNullOrEmpty(materialName) ) {
			for( int i = 0; i < mmdModel.materialList.Length; ++i ) {
				var material = mmdModel.materialList[i];
				if( material != null && !string.IsNullOrEmpty(material.surfaceName) ) {
					if( material.surfaceName == materialName ) {
						return i;
					}
				}
			}
		...
		if( materials != null ) {
			foreach( Material material in materials ) {
				...
				if( fbxAssetDirectoryName == Path.GetDirectoryName( Path.GetDirectoryName( materialPath ) ) ) {
					...
					int materialValue = _ToMaterialIndex( mmdModel, materialName );
					if( materialValue < 0 ) {
						materialValue = MMD4MecanimCommon.ToInt( materialName );
					}
	...
	public static List<Material> _GetFBXMaterialList( GameObject fbxAsset, MMDModel mmdModel )
	{
		if( fbxAsset == null ) {
			return null;
		}
		...
		if( meshRenderers != null ) {
			foreach( MeshRenderer meshRenderer in meshRenderers ) {
				int tempMaterialCount = _ComputeFBXMaterialList( meshRenderer.sharedMaterials, fbxAssetDirectoryName, materialDict, mmdModel );
		...
		if( skinnedMeshRenderers != null ) {
			foreach( SkinnedMeshRenderer skinnedMeshRenderer in skinnedMeshRenderers ) {
				int tempMaterialCount = _ComputeFBXMaterialList( skinnedMeshRenderer.sharedMaterials, fbxAssetDirectoryName, materialDict, mmdModel );
				if( materialCount < tempMaterialCount ) {
					materialCount = tempMaterialCount;
				}
			}
	...
	private void _OnInspectorGUI_Material()
	{
		...
		if( this.materialList == null ) {
			this.materialList = _GetFBXMaterialList( this.fbxAsset, this.mmdModel );
			if( this.materialList != null ) {
		...
		mmd4MecanimProperty.prefixRenderQueue = EditorGUILayout.Toggle( "PrefixRenderQueue", mmd4MecanimProperty.prefixRenderQueue );
		GUI.enabled = mmd4MecanimProperty.prefixRenderQueue;
		mmd4MecanimProperty.renderQueueAfterSkybox = EditorGUILayout.Toggle( "RenderQueueAfterSkybox", mmd4MecanimProperty.renderQueueAfterSkybox );
		GUI.enabled = true;
		mmd4MecanimProperty.baseDiffuse = EditorGUILayout.ColorField( "BaseDiffuse", mmd4MecanimProperty.baseDiffuse );
		mmd4MecanimProperty.ambientRate = EditorGUILayout.Slider( "AmbientRate", mmd4MecanimProperty.ambientRate, 0.0f, 2.0f );
		mmd4MecanimProperty.ambientToDiffuse = EditorGUILayout.Slider( "AmbientToDiffuse", mmd4MecanimProperty.ambientToDiffuse, 0.0f, 1.0f );
		...
		mmd4MecanimProperty.materialMorph = EditorGUILayout.Toggle( "MaterialMorph", mmd4MecanimProperty.materialMorph );
		mmd4MecanimProperty.shadowLum = EditorGUILayout.FloatField( "ShadowLum", mmd4MecanimProperty.shadowLum );
		mmd4MecanimProperty.isSpecular = EditorGUILayout.Toggle( "Specular", mmd4MecanimProperty.isSpecular );
	...
	static bool _Contains( string[] keywords, string keyword )
	{
		if( keywords != null ) {
			...
			if( material.shader != null &&
				...
				if( material.HasProperty("_Revision") ) {
					if( material.GetFloat("_Revision") < ShaderRevision - 0.5f ) {
						...
						#else
						if( material.IsKeywordEnabled("UNITY4SUPPORT_ON") ) {
							materialState |= MaterialState.Previous;
						} else {
							materialState |= MaterialState.Normally;
						}
