                    

namespace Regulus.Project.ItIsNotAGame1.Data.Adsorption
{
    using System.Linq;
        
    public class MoveControllerAdsorber : UnityEngine.MonoBehaviour , Regulus.Remoting.Unity.Adsorber<IMoveController>
    {
        private readonly Regulus.Utility.StageMachine _Machine;        
        
        public string Agent;

        private global::Regulus.Project.ItIsNotAGame.Data.Agent _Agent;

        [System.Serializable]
        public class UnityEnableEvent : UnityEngine.Events.UnityEvent<bool> {}
        public UnityEnableEvent EnableEvent;
        [System.Serializable]
        public class UnitySupplyEvent : UnityEngine.Events.UnityEvent<Regulus.Project.ItIsNotAGame1.Data.IMoveController> {}
        public UnitySupplyEvent SupplyEvent;
        Regulus.Project.ItIsNotAGame1.Data.IMoveController _MoveController;                        
       
        public MoveControllerAdsorber()
        {
            _Machine = new Regulus.Utility.StageMachine();
        }

        void Start()
        {
            _Machine.Push(new Regulus.Utility.SimpleStage(_ScanEnter, _ScanLeave, _ScanUpdate));
        }

        private void _ScanUpdate()
        {
            var agents = UnityEngine.GameObject.FindObjectsOfType<global::Regulus.Project.ItIsNotAGame.Data.Agent>();
            _Agent = agents.FirstOrDefault(d => string.IsNullOrEmpty(d.Name) == false && d.Name == Agent);
            if(_Agent != null)
            {
                _Machine.Push(new Regulus.Utility.SimpleStage(_DispatchEnter, _DispatchLeave));
            }            
        }

        private void _DispatchEnter()
        {
            _Agent.Distributor.Attach<IMoveController>(this);
        }

        private void _DispatchLeave()
        {
            _Agent.Distributor.Detach<IMoveController>(this);
        }

        private void _ScanLeave()
        {

        }


        private void _ScanEnter()
        {

        }

        void Update()
        {
            _Machine.Update();
        }

        void OnDestroy()
        {
            _Machine.Termination();
        }

        public Regulus.Project.ItIsNotAGame1.Data.IMoveController GetGPI()
        {
            return _MoveController;
        }
        public void Supply(Regulus.Project.ItIsNotAGame1.Data.IMoveController gpi)
        {
            _MoveController = gpi;
            
            EnableEvent.Invoke(true);
            SupplyEvent.Invoke(gpi);
        }

        public void Unsupply(Regulus.Project.ItIsNotAGame1.Data.IMoveController gpi)
        {
            EnableEvent.Invoke(false);
            
            _MoveController = null;
        }
        
        public void RunForward()
        {
            if(_MoveController != null)
            {
                _MoveController.RunForward();
            }
        }

        public void Forward()
        {
            if(_MoveController != null)
            {
                _MoveController.Forward();
            }
        }

        public void Backward()
        {
            if(_MoveController != null)
            {
                _MoveController.Backward();
            }
        }

        public void StopMove()
        {
            if(_MoveController != null)
            {
                _MoveController.StopMove();
            }
        }

        public void TrunLeft()
        {
            if(_MoveController != null)
            {
                _MoveController.TrunLeft();
            }
        }

        public void TrunRight()
        {
            if(_MoveController != null)
            {
                _MoveController.TrunRight();
            }
        }

        public void StopTrun()
        {
            if(_MoveController != null)
            {
                _MoveController.StopTrun();
            }
        }
        
        
    }
}
                    