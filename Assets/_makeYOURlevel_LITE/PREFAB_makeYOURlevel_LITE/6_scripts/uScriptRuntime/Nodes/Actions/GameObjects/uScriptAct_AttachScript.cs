         ...
         if ( !string.IsNullOrEmpty(currentScript) )
         {
            ...
            try
            {
               foreach ( GameObject currentGameObject in Target )
               {
                  ...
                  {
                     //UnityEngineInternal.APIUpdaterRuntimeServices.AddComponent(currentGameObject, "Assets/_makeYOURlevel_LITE/PREFAB_makeYOURlevel_LITE/6_scripts/uScriptRuntime/Nodes/Actions/GameObjects/uScriptAct_AttachScript.cs (52,22)", tempScript);
                  }
